...
void CScene::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	...
	pd3dDescriptorRanges[17].BaseShaderRegister = 14; //t13: 핑퐁용 Rtv
...
void CMainScene::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int mainPlayerId)
{
	...
	m_pPostProcessingShader->CreateResourcesAndRtvsSrvs(pd3dDevice, pd3dCommandList, ADD_RENDERTARGET_COUNT, pdxgiResourceFormats, d3dRtvCPUDescriptorHandle); //SRV to (Render Targets) + (Depth Buffer) + EMISSIVE + LowHDR
...
//void CMainScene::CreateComputeRootSignature(ID3D12Device* pd3dDevice)
//{
...
//	pd3dDevice->CreateRootSignature(0, pd3dSignatureBlob->GetBufferPointer(), pd3dSignatureBlob->GetBufferSize(), __uuidof(ID3D12RootSignature), (void**)m_pd3dComputeRootSignature.Get());
